...
import { useSolanaAgentProfile } from "@/hooks/useSolanaAgentProfile";
import { GRID_SIZE } from "@/config/contracts";
import type { SolanaPixel } from "@/lib/solana";

function colorForOwner(owner: string | null): string {
  if (!owner || owner === "11111111111111111111111111111111") return "#27272a";
  let h = 0;
...
interface SolanaPixelCellProps {
  id: number;
...
export function SolanaPixelCell({ id, data, pixelSize, onClick }: SolanaPixelCellProps) {
  const [hover, setHover] = useState(false);
  const [tooltipPos, setTooltipPos] = useState({ x: 0, y: 0 });
  const ref = useRef<HTMLButtonElement>(null);
  const x = Math.floor(id / GRID_SIZE);
  const y = id % GRID_SIZE;
  const ownerAddr = data?.owner && data.owner !== "11111111111111111111111111111111" ? data.owner : null;
  const profile = useSolanaAgentProfile(ownerAddr);
  ...
  const priceLabel = !data?.exists
    ? "1 USDC to claim"
    : data.forSale
      ? `${(data.price / 1_000_000).toFixed(2)} USDC`
      : null;
